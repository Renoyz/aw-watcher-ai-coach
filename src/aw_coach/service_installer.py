...
class ServiceManager:
    ...
    def status(self) -> ServiceStatus:
        self._require_windows()
        result = self._run_powershell(self._status_script(), check=True)
    ...
    def _install_script(self) -> str:
        argument = self._quote_ps_double(self._powershell_launch_arguments())
        return (
    ...
    def _install_runkey_script(self) -> str:
        run_key_path = self._quote_ps(self._run_key_path())
        run_key_name = self._quote_ps(self._run_key_name())
        run_key_value = self._quote_ps(self._run_key_value())
        return (
            f"New-Item -Path {run_key_path} -Force | Out-Null; "
            f"New-ItemProperty -Path {run_key_path} -Name {run_key_name} "
            f"-Value {run_key_value} -PropertyType String -Force"
        )

    def _start_script(self) -> str:
        run_key_path = self._quote_ps(self._run_key_path())
        run_key_name = self._quote_ps(self._run_key_name())
        launch_arguments = self._quote_ps(self._powershell_launch_arguments())
        return (
            "$task = Get-ScheduledTask -TaskName 'aw-coach' -ErrorAction SilentlyContinue; "
            "if ($task) { "
            "Start-ScheduledTask -TaskName 'aw-coach' "
            "} else { "
            f"$runEntry = Get-ItemProperty -Path {run_key_path} -Name {run_key_name} "
            "-ErrorAction SilentlyContinue; "
            "if ($runEntry) { "
            "Start-Process -FilePath 'powershell.exe' "
            f"-ArgumentList {launch_arguments} -WindowStyle Hidden "
            "}}"
        )
    ...
    def _status_script(self) -> str:
        ...
        return (
            ...
            f"elseif (Get-ItemProperty -Path {run_key_path} -Name {run_key_name} "
    ...
    def _uninstall_script(self) -> str:
        ...
        return (
            ...
            f"Remove-ItemProperty -Path {run_key_path} -Name {run_key_name} "
            "-ErrorAction SilentlyContinue"
        )
    ...
    def _powershell_launch_arguments(self) -> str:
        return (
    ...
    def _powershell_launch_command(self) -> str:
        return "powershell.exe " + self._powershell_launch_arguments()
    ...
    def _quote_ps(self, value: Union[Path, str]) -> str:
