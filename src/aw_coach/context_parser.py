...
class TitleParser:
    ...
    _IDE_SEPARATORS = re.compile(r"\s+[-—|·•]\s+")
    ...
    _TERMINAL_LEADING_MARKS = re.compile(r"^[^\w~/\\]+")
    _TERMINAL_PROJECT_TOKEN = re.compile(r"[\w.][\w.-]*")
    ...
    def parse(self, app: str, title: str, url: Optional[str] = None) -> WindowContext:
        ...
        project = None if self._is_browser(app) and not url else self._extract_project(title, url)
        if project is None and not url and self._is_terminal(app):
            project = self._extract_terminal_project(title)
    ...
    @classmethod
    def _extract_terminal_project(cls, title: str) -> Optional[str]:
        candidate = cls._TERMINAL_LEADING_MARKS.sub("", title.strip()).strip()
        if not candidate or cls._is_noise_word(candidate):
            return None
        if cls._TERMINAL_PROJECT_TOKEN.fullmatch(candidate):
            return candidate
        return None
    ...
    def _is_browser(cls, app: str) -> bool:
        return cls._normalize_app(app) in cls._BROWSER_APPS

    @classmethod
    def _is_ide(cls, app: str) -> bool:
        return cls._normalize_app(app) in cls._IDE_APPS

    @classmethod
    def _is_terminal(cls, app: str) -> bool:
        return cls._normalize_app(app) in cls._TERMINAL_APPS
