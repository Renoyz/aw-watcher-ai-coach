...
class TestTitleParserBrowser:
    ...
    def test_windows_browser_exe_suffix_does_not_create_project_from_app_name(self):
        p = TitleParser()
        ctx = p.parse("chrome.exe", "V2EX - Google Chrome", url=None)
        assert ctx.project is None
...
class TestTitleParserTerminal:
    ...
    def test_terminal_hyphenated_project_name(self):
        p = TitleParser()
        ctx = p.parse("WindowsTerminal.exe", "aw-watcher-ai-coach")
        assert ctx.project == "aw-watcher-ai-coach"
