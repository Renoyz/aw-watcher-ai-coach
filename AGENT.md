...
On Windows, `aw-coach service install` registers a Task Scheduler task named `aw-coach` for the current user.
It runs `aw-coach-daemon` at logon in the current user session, preserving desktop notifications and ActivityWatch local API access.
If Task Scheduler registration is denied for the current user, `aw-coach service install` falls back to using an HKCU Run key entry for automatic startup.
...
| 截图 | 默认触发式启用，不落盘原图 | 只在 unknown/stuck/长时间同标题时分析，保留结果不保留截图 |
...
不存在时使用全部默认值（hybrid 模式）。如果未配置 API key，会自动降级为 rule_only，零配置仍可用。

关键配置项：
```toml
[ai]
backend = "hybrid"  # rule_only / openai / hybrid

[ai.openai]
api_key = "sk-..."
base_url = "https://api.deepseek.com/v1"  # 兼容 OpenAI API 的任何服务
model = "deepseek-v4-flash"

[cost]
monthly_budget_usd = 2.86
...
4. Policy Engine / Agent Inbox 尚未实现，主动建议仍以通知和 CLI 为主

## 下一步路线（Phase 5: 主动 Agent）

当前系统处于 **L2→L3 过渡**：分类、总结、实时语义状态和 Context Stack 已部分落地。下一步目标是升级到 **L4 级**（主动建议动作）。

核心缺失模块（按优先级）：

1. **Detector 层** (`detector.py`) — unknown/high_switch/ai_coding/focus_block
2. **Policy Engine** (`policy.py`) — interrupt budget + quiet hours + focus protect
3. **Agent Inbox** (`inbox.py`) — 建议收件箱 + accept/dismiss
4. **Feedback Memory** (`profile.py`) — user_profile + 学习衰减
